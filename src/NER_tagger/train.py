...
import src.NER_tagger.loader as loader
import torch
import time
import pickle
...
from src.NER_tagger.model import BiLSTM_CRF
...
print('the length of word_to_id: ', len(word_to_id))
# build the model
...
def evaluating(model, datas, best_F):
    ...
    for data in datas:
        ...
        if parameters['char_mode'] == 'LSTM':
            ...
            chars2_mask = np.zeros((len(chars2_sorted), char_maxl), dtype='int64')
        ...
        if parameters['char_mode'] == 'CNN':
            ...
            char_maxl = max(chars2_length)
            chars2_mask = np.zeros((len(chars2_length), char_maxl), dtype='int64')
    ...
    with open(predf, 'a') as f:
        f.write('\n'.join(prediction))    # 将预测结果写入文件
    print('%s <%s >%s' % (eval_script, predf, scoref))  # 使用os.system执行conlleval脚本
...
for epoch in range(1, 10001):
    for i, index in enumerate(np.random.permutation(len(train_data))):
        ...
        if parameters['char_mode'] == 'LSTM':
            ...
            for i, c in enumerate(chars2_sorted):
                chars2_mask[i, :chars2_length[i]] = c
            chars2_mask = Variable(torch.from_numpy(chars2_mask))
        ...
        if parameters['char_mode'] == 'CNN':
            ...
            for i, c in enumerate(chars2):
                chars2_mask[i, :chars2_length[i]] = c
            # print(chars2_mask)
            chars2_mask = Variable(torch.from_numpy(chars2_mask))  # torch.LongTensor()
        ...
        torch.nn.utils.clip_grad_norm(model.parameters(), 5.0)  # 梯度裁剪
        optimizer.step()

        if count % plot_every == 0:
            loss /= plot_every
            print("count %i, loss %f" % (count, loss))
            if losses == []:
                losses.append(loss)
            losses.append(loss)
