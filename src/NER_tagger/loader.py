...
import src.NER_tagger.model as model
import string
import random
import numpy as np
    ...
    for data in datas[start:start+batch_size]:
        ...
        for word in data['words']:
            if word in singletons and np.random.uniform() < 0.5:
                words.append(1)  # 以0.5的概率添加频率为1的单词
            else:
                words.append(word)
...
def random_batch(batch_size, train_data, singletons=[]):
    """
    获取batch_size的数据， 随机获取一个batch的大小
    :param batch_size:
    :param train_data:
    :param singletons:
    :return:
