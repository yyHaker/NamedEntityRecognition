...
class Optimization:
    ...
    def get_gradients(self, cost, params):
        ...
        if self.clip is None:
            ...
            assert self.clip > 0
            return T.grad(theano.gradient.grad_clip(cost, -1*self.clip, self.clip), params)
    ...
    def sgdmomentum(self, cost, params, lr=0.01, momentum=0.9):
        """
        Stochastic gradient decent with momentum. Momentum has to be in [0,1).
        :param cost:
        :param params:
        :param lr:
        :param momentum:
        :return:
        """
        # Check that the momentum ia s correct value
        assert 0 <= momentum < 1
    ...
    def adagrad(self, cost, params, lr=1.0, epsilon=1e-6):
        ...
        """
        lr = theano.shared(np.float32(lr).astype(floatX))
        epsilon = theano.shared(np.float32(lr).astype(floatX))
    ...
    def adam(self, cost, params, lr=0.01, beta1=0.9, beta2=0.999, epsilon=1e-8):
        """
        ...
        for param, gradient in zip(params, gradients):
            ...
            m_prev = theano.shared(value=np.zeros(value.shape, dtype=value.dtype), broadcastable=param.broadcastable)
            v_prev = theano.shared(value=np.zeros(value.shape, dtype=value.dtype), broadcastable=param.broadcastable)
