...
class Model(object):
    ...
    def __init__(self, parameters=None, models_path=None, model_path=None):
        """
        Initialize the model. We either provide the parameters and a path where we store the models,
        or the location of trained model.
        :param parameters:
        :param models_path: 存储模型参数的路径
        :param model_path: 存储训练好的模型的路径
        """
    ...
    def add_component(self, param):
        ...
        if param.name in self.components:
            raise Exception('The network already has a parameter %s !' % param.name)
    ...
    def build(self, dropout, char_dim, char_lstm_dim, char_bidirect, word_dim, word_lstm_dim, word_bidirect,
              lr_method, pre_emb, crf, cap_dim, training=True, **kwargs):
        """
        Build the network.
        :param dropout:
        :param char_dim:
        ...
        """
        # Training parameters
        n_words = len(self.id_to_word)  # the number of words
        ...
        char_for_ids = T.imatrix(name='char_for_ids')  # char forward ids
        char_rev_ids = T.imatrix(name='char_rev_ids')  # char reverse ids
        char_pos_ids = T.imatrix(name='char_pos_ids')
        ...
        if word_dim:
            ...
            if pre_emb and training:
                ...
                for i, line in enumerate(codecs.open(pre_emb, 'r', 'utf-8')):
                    line = line.rstrip().split()  # 去除后缀，按照空格分割
                ...
                print '%i found directly, %i after lowercasting, %i after lowercasting + zero' % (
        ...
        if char_dim:
            ...
            # h (sequence_length, batch_size, output_dim)  char_pos_ids?
        ...
        tags_scores = final_layer.link(final_output)     # tag_scores (sequence_length, n_tags) ?

        # No CRF
        if not crf:
            cost = T.nnet.categorical_crossentropy(tags_scores, tag_ids).mean()
        # CRF
        # observations: sequence as shape (n_steps, n_classes), observations[i][j]表示第i个词标注为第j个tag的分数
        # transitions: sequence as shape (n_classes, n_classes), transitions[i][j]表示第i个tag->第j个tag的转移分数
        else:
            transitions = shared((n_tags + 2, n_tags + 2), name='transitions')
            small = -1000
            b_s = np.array([[small] * n_tags + [0, small]]).astype(np.float32)
            e_s = np.array([[small] * n_tags + [small, 0]]).astype(np.float32)

            observations = T.concatenate([tags_scores, small * T.ones((s_len, 2))], axis=1)  # 在行上添加
            observations = T.concatenate([b_s, observations, e_s], axis=0)   # 在列上添加 -> (sequence_length+2, n_tags+2)
            ...
            real_path_score += transitions[padded_tags_ids[T.arange(s_len+1)], padded_tags_ids[T.arange(s_len+1)+1]].sum()

            all_paths_scores = forward(observations, transitions)
            cost = - (real_path_score - all_paths_scores)  # log(p(y|X))=s(X,y)-logAdds(X,y_)
        ...
        print 'Compiling...'
        if training:
            updates = Optimization(clip=0.5).get_updates(lr_method_name, cost, params, **lr_method_parameters)
            f_train = theano.function(inputs=train_inputs, outputs=cost, updates=updates,
        ...
        if not crf:
            f_eval = theano.function(inputs=eval_inputs, outputs=tags_scores,
                                     givens=({is_train: np.cast['int32'](1)} if dropout else {}))
        else:
            f_eval = theano.function(inputs=eval_inputs, outputs=forward(observations, transitions, viterbi=True,
